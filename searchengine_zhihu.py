        ...
        if data_list:
            for one in data_list:
                if one["type"] == "search_result":
                    try:
                        title = one["object"]["question"]["name"].replace("<em>", "").replace("</em>", "").strip()
                        ...
                        release_date = one["object"]['created_time']

                        items['tag'] = '知乎'
                        items['target'] = href
                        items['source'] = None
                        items['releaseAt'] = datetime.fromtimestamp(int(release_date))
                        items['title'] = title
